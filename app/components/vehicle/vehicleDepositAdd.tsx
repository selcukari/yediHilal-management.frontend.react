...
export type VehicleDepositAddDialogControllerRef = {
  openDialog: (vehicleId: number, vehiclePlate: string) => void;
  close: () => void;
};
...
const VehicleDepositAdd = forwardRef<VehicleDepositAddDialogControllerRef, UserAddProps>(({onSaveSuccess}, ref) => {
  const [isDisabledSubmit, setIsDisabledSubmit] = useState(false);
  const [vehicle, setVehicleId] = useState({id: 0, plate: ''});
  const [opened, { open, close }] = useDisclosure(false);
  const service = useUserService(import.meta.env.VITE_APP_API_VEHICLE_CONTROLLER);
  ...
  const openDialog = (vehicleId: number, vehiclePlate: string) => {

    if (vehicleId && vehicleId > 0 && vehiclePlate) {
      setVehicleId({id: vehicleId, plate: vehiclePlate});
      open();
    }
  }
      ...
      <form onSubmit={form.onSubmit(handleSubmit)}>
        <Stack gap="md">
          <Grid>
            <Grid.Col span={6}>
              <TextInput
                label="Araç Plaka" disabled required value={vehicle.plate}/>
            </Grid.Col>
            <Grid.Col span={6}>
              <TextInput
                label="Ad Soyad"
